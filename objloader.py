...
class OBJ:
    ...
    def loadMaterial(cls, filename):
        ...
        with open(filename, "r") as f:
            for line in f.read().split('\n'):
                #if there is no content in this line, read the content of the next line.
    ...
    def __init__(self,filename : str,swapyz : bool = False) -> None:
        self.vertices = []
        self.normals = []
        self.texcoords = []
        self.faces = []
    ...
    def get_min_max(self) -> tuple:
        ...
        z_max = max([vertice[2] for vertice in self.vertices])
        x_min = min([vertice[0] for vertice in self.vertices])
        y_min = min([vertice[1] for vertice in self.vertices])
        z_min = min([vertice[2] for vertice in self.vertices])
